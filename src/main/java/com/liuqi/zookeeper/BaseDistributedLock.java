...
public abstract class BaseDistributedLock implements DistributedLock{
	...
	private void init(){
		try {
			Stat stat = zk.exists(rootPath, false);		//判断根目录是否存在
			if(null == stat){
		...
		try {
			while (!haveTheLock){
				...
				if(isGetTheLock){
					...
					// 如果节点不存在会出现异常
					zk.exists(previousSequencePath, previousListener);
	...
	private String createLockNode(String path) throws Exception{
		...
		return zk.create(path, null, ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.EPHEMERAL_SEQUENTIAL);
	}
	 ...
	 * @throws Exception
	 */
	protected Boolean attempLock(long time, TimeUnit unit) throws Exception{
